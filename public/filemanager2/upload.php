...
// try {
    ...
    $messages = null;
    if (xtrans("Upload_error_messages") !== "Upload_error_messages") {
        $messages = xtrans("Upload_error_messages");
    }
    ...
    if (!checkresultingsize($_FILES['files']['size'][0])) {
        ...
        $upload_handler->response['files'][0]->error = sprintf(xtrans('max_size_reached'), $config['MaxSizeTotal']) . AddErrorLocation();
